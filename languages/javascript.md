...
- **Scope chain** is a tree that maps the nesting of scopes within a function
  `(global) ↑ | foo() var bar ↑ | zip() var quux`
...
- instructions are called _statements_ and are seperated by a semicolon(;)
...
- var, let and const are used to declare variables
  ...
  - variables declared by var or let that have no initial value are set to _undefined_
...
- Expressions containing the _+_ operator with numeric and string value automatically convert the numerics to strings
...
- _debugger_ can be used to invoke any available debugger if one exists ie like setting a breakpoint
...
- The simplest way to create an object is by using the object literal syntax:
  `var obj = { prop: value };`
- the \***\*proto\*\*** property can be used to create a new object from the prototype of an existing object, its available in ES6 onwards
...
- An object can be made iterable by assigning to its @@iterator property, through the Symbol.iterator
  `let foo = { [Symbol.iterator]: () => ({ items: ['a','b','c'], next: function next () { return { done: this.items.length === 0, value: this.items.shift() } } }); }`
...
## Transpilation, babel etc

- Babel generates an AST (Abstract syntax tree) of the source code
- Transformations can be applied to the AST
- Codemods can be single use plugins executed on source code. Useful for migrations and upgrades
- ESLint plugins useful for inspecting and reporting on the source code
- Babel plugins, useful to transform the source code
- ESLint plugins receive a node from the source code as a context, Babel plugins receive a path object as context which includes the node
...
- [Custom babel and ESLint plugins](https://www.youtube.com/watch?v=VicU_XwriWw)
